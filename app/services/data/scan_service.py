"""
ScanService — read-only data quality scan.

Detects: missing values, disguised missing tokens, outliers, duplicates.
Returns a flat list of issues with row numbers and descriptions,
ready to be displayed in the frontend error sheet BEFORE cleaning.

Uses RelationshipDetector._is_identifier() to exclude identifier/key columns
from outlier detection (StockCode, CustomerID, InvoiceNo, etc.)
"""
...
class ScanService:
    ...
    # Name patterns that should NEVER be treated as numeric metrics for outlier detection.
    # This is intentionally broader than RelationshipDetector._is_identifier()
    # which also requires high uniqueness (for relationship analysis).
    # For outlier exclusion, the name alone is sufficient.
    ...
    def _get_identifier_columns(df: pd.DataFrame) -> List[str]:
        ...
        for col in df.columns:
            # 1. Shared cleaning heuristic. Metric-looking names such as
            # Quantity, UnitPrice, or DiscountPercent should not be skipped just
            # because they are unique in a small sample.
    ...
    def scan_dataframe(
        ...
        # ── 1. True missing values (NaN / None) ─────────────────────────
        for col in df.columns:
        ...
        # ── 2. Disguised missing tokens ("na", "?", "-", etc.) ──────────
        ...
        # ── 3. Outliers — reuse _handle_outliers (read-only via copy) ───
        # Exclude identifier/key columns using the existing RelationshipDetector
        # heuristic (same logic used by ColumnAnalyzer).
        try:
