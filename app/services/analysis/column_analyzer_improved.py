...
Extends the basic ``ColumnAnalyzer`` with statistical tests for:
- Categorical–categorical relationships (chi-square, Cramér's V)
- Numeric–categorical relationships (ANOVA, eta-squared)

All returned relationships include:
- ``strength``: effect size (Cramér's V or eta-squared)
- ``p_value``: statistical significance
- ``statistic``: chi-square or F statistic
- ``significance``: '*', '**', or '***' based on p-value
"""
...
class ImprovedColumnAnalyzer(ColumnAnalyzer):
    ...
    _CRAMERS_V_THRESHOLD: float = 0.10  # small effect
    _ETA_SQUARED_THRESHOLD: float = 0.06  # medium effect
    ...
    def analyze(self) -> Dict:
        ...
        # 6. Select dashboard columns, primary metric, confidence
        ...
        for i in range(len(dimensions)):
            for j in range(i + 1, len(dimensions)):
                ...
                try:
                    ...
                    # Handle small cell counts: skip tables with any cell < 5
        ...
        for num_col in metrics:
            for cat_col in dimensions:
                try:
                    ...
                    # Eta-squared via statsmodels anova_lm
    ...
    def _calculate_eta_squared(
        ...
        try:
            ...
            # Assumes first row is the factor, second is residual
    ...
    def _merge_multiple_relationship_sets(
        ...
        for rel_list in rel_lists:
            for rel in rel_list:
                ...
                if not pair:
                    continue
                # Overwrite to prefer later (usually more detailed) relationships
                seen_pairs[pair] = rel

        # Preserve a deterministic order: sort by strength descending
