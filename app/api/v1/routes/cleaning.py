...
async def clean_dataset(
    ...
    if profile_id:
        ...
        if not profile:
            ...
            # Auto-upgrade older default profiles to the smarter defaults so users
            # don't get stuck with the old destructive behaviour.
            needs_upgrade = (
                getattr(profile, "is_default", False)
                and getattr(profile, "name", "") == "Default Cleaning Profile"
    ...
    if save_as_new:
        ...
        # Overwrite existing file but preserve the original via a timestamped backup
        try:
...
async def get_data_quality(
    ...
    #   missing 40 / duplicates 20 / outliers 20 / validation 20
...
async def apply_outliers_action(
    ...
    if requested_rows:
        ...
        # 1) Treat values as raw dataframe indexes.
        selected_rows.update(r for r in requested_rows_set if r in df.index)

        # 2) Treat values as 1-based row numbers (excluding CSV header).
        selected_rows.update((r - 1) for r in requested_rows_set if (r - 1) in df.index and r >= 1)

        # 3) Treat values as business row IDs when an `id` column exists.
        if "id" in df.columns:
    ...
    for col, col_meta in metadata.items():
        ...
        # If client did not specify selected_rows, apply to all current rows that
        # violate bounds in the chosen column (more robust than stale metadata row ids).
        if not selected_rows:
